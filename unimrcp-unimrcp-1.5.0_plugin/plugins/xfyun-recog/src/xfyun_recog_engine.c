...
struct xfyun_recog_channel_t {
    ...
    int                     null_recog_times;
    apt_bool_t              recog_finished;
...
{
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 5c6e4433, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
...
static mrcp_engine_channel_t* xfyun_recog_engine_channel_create(mrcp_engine_t *engine, apr_pool_t *pool)
{
    ...
    recog_channel->null_recog_times = 0;
    recog_channel->recog_finished = FALSE;
...
static apt_bool_t xfyun_recog_channel_recognize(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	const char*	session_begin_params = "sub = iat, domain = iat, language = zh_cn, accent = mandarin, sample_rate = 8000, result_type = plain, result_encoding = utf8, vad_eos = 500";
...
static apt_bool_t xfyun_recog_stream_recog(xfyun_recog_channel_t *recog_channel,
	...
	if (MSP_SUCCESS != ret)
	{
        ...
        recog_channel->recog_finished = TRUE;
		return FALSE;
	}
	...
	while (1) 
	{
		...
		if (NULL != rslt)
		{
            ...
            recog_channel->null_recog_times++;
        }
    ...
    if (ep_stat == MSP_EP_AFTER_SPEECH) {
        apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"[xfyun] end point detected");
        recog_channel->recog_finished = TRUE;
    }
	return TRUE;
}
...
static apt_bool_t xfyun_recog_stream_write(mpf_audio_stream_t *stream, const mpf_frame_t *frame)
{
	...
	if (recog_channel->last_result && (recog_channel->null_recog_times || recog_channel->recog_finished)) {
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"[xfyun] recog success.");
		xfyun_recog_recognition_complete(recog_channel,RECOGNIZER_COMPLETION_CAUSE_SUCCESS);
	}
	return TRUE;
}
